...
@jwt_required()
def get_messages(id):
    """Placeholder - messages are handled primarily via socket and perhaps archived later."""
    return success([])
...
def complete_consultation(id):
    ...
    # CLEANUP: Delete all messages and images from this consultation (temp storage only)
    try:
        from app.models.message import Message
        messages = Message.query.filter_by(consultation_id=id).all()
            ...
            for msg in messages:
                if msg.image_url and 'cloudinary.com' in msg.image_url:
                    try:
                        ...
                        # URL format: https://res.cloudinary.com/dngfwmb46/image/upload/v1234/telehealth/xxx.jpg
                        import re
                        match = re.search(r'\/v\d+\/(.+?)\.\w+$', msg.image_url)
                        if match:
        ...
        db.session.commit()
        print(f"✅ Cleaned up {len(messages)} messages for consultation {id}")
    except Exception as e:
