...
class RedisChatService {
        ...
        try {
          ...
          if (res.ok) {
            ...
            messages.forEach(msg => {
              if (!this.lastMessageId[consultationId] || msg.id > this.lastMessageId[consultationId]) {
                this.lastMessageId[consultationId] = msg.id;
  ...
  async sendMessage(consultationId, payload) {
    ...
    try {
      const res = await fetch(`/api/consultations/${consultationId}/message`, {
        method: 'POST',
        headers: {
  ...
  async syncLocalQueue() {
    ...
    for (const message of queue) {
      try {
        const res = await fetch(`/api/consultations/${message.consultation_id}/message`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
