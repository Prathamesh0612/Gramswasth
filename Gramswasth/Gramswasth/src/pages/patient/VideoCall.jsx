...
export default function VideoCall() {
  ...
  useEffect(() => {
    ...
    socketService.on('consultation_status_update', (data) => {
      if (data.consultation_id === consultationId && data.status === 'completed') {
        console.log("Consultation marked as completed by doctor");
        setRemoteEnded(true);
      }
    });
  ...
  // Socket.IO-based Chat + History Loading
  useEffect(() => {
    // 1. Load chat history
    consultationAPI.getMessages(consultationId).then(res => {
      if (res.success && res.data) {
        const history = res.data.map(m => ({
          sender: m.sender_id === userId ? 'me' : (userRole === 'doctor' ? 'patient' : 'doctor'),
          text: m.text,
          image: m.image,
      ...
      try {
        ...
        stats.forEach(report => {
          ...
          if (report.type === 'outbound-rtp' && report.kind === 'video') {
            ...
            // If upload <250kbps, switch to audio-only (very slow network)
            if (kbpsOut < 250 && !isAutoOffRef.current && !camOff) {
              console.log(`⚡ UPLOAD SLOW: ${kbpsOut.toFixed(0)}kbps - Disabling video INSTANTLY`);
              isAutoOffRef.current = true;
              ultraLowBandwidthRef.current = (kbpsOut < 100);
              setCameraError(`📱 Slow Upload (${kbpsOut.toFixed(0)}kbps): Audio Mode`);
              toggleCam(true);
            }
          }
          ...
          if (report.type === 'inbound-rtp' && report.kind === 'video') {
            ...
            if (lastBytesRef.current > 0) {
              ...
              // INSTANT AUDIO-ONLY: Bandwidth below 250 kbps - immediately disable camera
              if (kbps < 250 && !isAutoOffRef.current && !camOff) {
                console.log(`⚡ SLOW NETWORK: ${kbps.toFixed(0)}kbps - Disabling video INSTANTLY`);
                isAutoOffRef.current = true;
                ultraLowBandwidthRef.current = (kbps < 100); // Mark critical if <100
                setCameraError(`📱 Slow Network (${kbps.toFixed(0)}kbps): Audio Mode`);
                toggleCam(true);
              }
              // RECOVERY: Network improved to >600 kbps
              else if (kbps > 600 && isAutoOffRef.current) {
                console.log(`✅ Network recovered: ${kbps.toFixed(0)}kbps - Re-enabling video`);
  ...
  const toggleCam = (forceState) => {
    ...
    if (!stream) {
      ...
      if (shouldTurnOff) {
        setCameraError('📱 Video disabled for network stability');
      } else {
        setCameraError(null);
      }
      return;
    ...
    console.log(`Camera ${shouldTurnOff ? 'OFF' : 'ON'}`);
  };
  ...
  const handleExitCall = () => {
    // Notify peer
    socketService.emit('end_call', { consultation_id: consultationId });
    endCall();
  ...
  return (
    <div className="h-dvh max-h-dvh bg-gray-950 flex flex-col relative overflow-hidden">
        ...
        {callStatus !== 'ended' && (
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <button onClick={() => setShowExitConfirm(true)}
              className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center shadow-xl transition-all transform hover:scale-105 border-2 ${showExitConfirm ? 'bg-orange-500 border-orange-400/50' : 'bg-red-500 border-red-400/50 hover:bg-red-600'}`}>
      ...
      {/* Remote Ended Confirmation Modal */}
      <AnimatePresence>
        {remoteEnded && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/60 flex items-center justify-center z-[70] backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              className="bg-white rounded-3xl p-8 max-w-sm mx-4 shadow-2xl text-center border border-gray-100">
              <div className="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-4">
                <PhoneOff className="text-amber-500" size={32} />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-2">Consultation Ended</h3>
              <p className="text-sm text-gray-500 mb-8 leading-relaxed">
                The doctor has finalized this consultation session. You can now safely return to your dashboard.
              </p>
              <button onClick={handleExitCall}
                className="w-full py-4 rounded-2xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200 active:scale-95">
                Go to Dashboard
              </button>
            </motion.div>
          </motion.div>
